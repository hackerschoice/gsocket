...
struct _gopt
{
	...
	char *prg_exename;      // argv[0] or link destination of /proc/self/exe. Used to warn user if deleted. Executeable (maybe bin-encrypted) on filesystem.
	char *proc_hiddenname;  // argv[0]. Overwritten by config.
	//char *config_fsname;    // /proc/self/exe or file on the filesystem (if not bincrypted)
	char *swd_str;
...
#define GSC_FL_MEMEXEC              (0x400000) // memexec() is known to fail. Do not use it.
